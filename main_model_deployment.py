...
from rl.agent import DefaultAgent, PolicyAgent
...
class TrainSimulation(arcade.Window):
    ...
    def __init__(self, width, height, title, average_track_length, num_transitions, num_trains, model_file):
        ...
        policy = DDDQNPolicy(252, 4, model_params, False)
        policy.load(model_file)
        for i, train in enumerate(self.simulation.train_fleet):
            train.set_agent(PolicyAgent(train, TreeObserver(train, self.simulation, 3), policy, 0.0, self.simulation.rng))
